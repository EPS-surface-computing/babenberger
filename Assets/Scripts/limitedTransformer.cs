...
namespace TouchScript.Behaviors
{
    ...
    public class limitedTransformer : MonoBehaviour
    {
        // Here's how it works.
        //
		...
		//in ui a float can be entered for min max zoom in the program it is automaticaly put in a vector3


		/// <summary>
		/// Sets the max scale of the image
		/// </summary>
		...
		//these where made into property's so we could limit them
		...
		{
			get { return targetScl;}
			set { targetScl = scaleLimiter(value);}
		}
		...
		#region Limiting functions
		
		private Vector3 scaleLimiter(Vector3 desiredScale){
			Vector3 bs = desiredScale;
			desiredScale = Vector3.Max(new Vector3(minScale,minScale,minScale),desiredScale);
			desiredScale = Vector3.Min(new Vector3(maxScale,maxScale,maxScale),desiredScale);
			if(!bs.Equals(desiredScale)){
				targetPosition = currentPosition + (gesture.DeltaPosition*0.5f);
				return Vector3.Lerp(bs,desiredScale,0.5f);
			}
			//Debug.Log(gesture.DeltaPosition);
			if(desiredScale.x.Equals(minScale)){
        ...
        private void stateIdle()
        {
            ...
            {
                transform.position = lastPosition = targetPos;
        ...
        private void stateManual()
        {
            ...
            targetPos = lastPosition = cachedTransform.position;
        ...
        private void update()
        {
            ...
            if ((transformMask & TransformGesture.TransformType.Translation) != 0)
            {
                ...
                //         targetPos.z = pos.z;
                // }
                transform.position = Vector3.Lerp(pos, targetPos, fraction);
                // Something might have adjusted our position (most likely Unity UI).
                lastPosition = transform.position;

                if (state == TransformerState.Automatic && !changed && (targetPos - lastPosition).sqrMagnitude > positionThreshold) changed = true;
            }
        ...
        private void manualUpdate()
        {
			...
			if ((mask & TransformGesture.TransformType.Translation) != 0) targetPos += gesture.DeltaPosition;
            transformMask |= mask;
            ...
            gesture.OverrideTargetPosition(targetPos);
        ...
        private void applyValues()
        {
            ...
            if ((transformMask & TransformGesture.TransformType.Translation) != 0) cachedTransform.position = targetPos;
            transformMask = TransformGesture.TransformType.None;
        }
